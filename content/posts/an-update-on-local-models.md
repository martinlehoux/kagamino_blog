...
This week was interesting for local models, because we had the
open weights release of Qwen 3.8 27B. It was much expected, because
...
To think that I could run as a powerful model locally feels
insane.
For reference, it sits between a Claude Opus 4.6 and a
Claude Opus 4.7, and that was the best what we had in February
in Claude Code.

## Artificial Analysis pareto frontier

https://artificialanalysis.ai is my reference for model comparison.
It actually compiles scores from many benchmarks. I know there are
many pitfalls to these benchmarks, but hey, we have to reduce
the world's complexity, right?
I found my prefered charts, and that's when you remove all models.
Then AAI highlights the pareto frontier, with the best option
at each tradeoff. Go see for yourself!


[Int vs Cost/Time per task](https://artificialanalysis.ai/models#intelligence-comparisons) - What frontier model to use in
what case. Here we can see Luna is pretty neat for cost, and
...
run on my hardware. For this one, you need to filter on small
(and maybe tiny) models. This show Qwen 3.8 27B as much better
...
I wonder if local inference got better since https://blog.kagamino.dev/posts/local-llm-inference-offline-scenario/.
...
I'll go by feel, because I don't have clear metrics to target.

Sadly, Qwen 3.8 27B is too large, even with oMLX. That's also
because I need a larger context of 100k tokens. And I couln't
run Ling 3.0 Tiny, because of unsupported architecture. So much
for modern models!
...
With oMLX and a 8bits version, it runs using around 10GB
